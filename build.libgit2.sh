...
if [[ $OS == "Darwin" ]]; then
    ...
    if [[ $RID == android-* ]]; then
        ...
        if [[ $SSL_PATH == "" ]]; then
            SSL_PATH=$PWD/OpenSSL/$ABI
        fi

        CMAKE_ANDROID=" -DOPENSSL_INCLUDE_DIR=$SSL_PATH/include/ \
                        -DOPENSSL_SSL_LIBRARY=$SSL_PATH/libssl.a \
                        -DOPENSSL_CRYPTO_LIBRARY=$SSL_PATH/libcrypto.a \
